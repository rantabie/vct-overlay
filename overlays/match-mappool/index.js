(function () {
  ...
  function extractLiveState(data) {
    ...
    const activeMap = extractLiveMap(data, clients);
  ...
  function extractLiveMap(data, clients = []) {
  ...
  function normaliseIpcState(value) {
    ...
    if (text.includes("play")) return GAMEPLAY_STATE;
    if (text.includes("result") || text.includes("ranking")) return RESULT_STATE;
    return 0;
  }
  ...
  function formatMapMeta(map) {
    ...
    return [
      ...
      sr ? `SR ${formatSrNumber(sr)}${map.moddedSr ? "*" : ""}` : "",
      formatMapBpm(map),
      formatMapLength(map),
